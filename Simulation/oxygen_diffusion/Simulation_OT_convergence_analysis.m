...
pO2_n_result_ds2_nn.pO2_array_us = imresize3(pO2_n_result_ds2_nn.pO2_array, int_bbox_exp_size, ...
    'Method', 'linear');
...
% Because they converge from different direction
...
% Conclusion: 
% uint8 conversion + linear / cubic interpolation seems to give the closest
% DT distribution
%
...
% 1. Linear interpolation: 2x downsampled solution after linear upsampling
% is on average 8.29% smaller than 1 um resolution solution (median -
% 7.95%)
% 2. Nearest neighbor interpolation: 2x downsampled solution after linear
% upsampling is on average 12.14% larger than 1 um resolution solution
% (median 9.03%)
tmp_x = pO2_n_result_ds2_ln.pO2_array_us;
tmp_y = pO2_n_result_ln_2.pO2_array;
% tmp_x = pO2_n_result_ds2_nn.pO2_array_us;
% tmp_y = pO2_n_result_nn_2.pO2_array;
ds_err_n = fun_error_n(tmp_x, tmp_y);
fig_hdl = figure;
ax_hdl = axes(fig_hdl);
...
ax_hdl.YScale = 'log';
    %%
dt_bin_um = 0 : 2 : 60;
y_in_x_str = fun_analysis_get_y_stat_in_x_bin(vessel_recon_mask_dt, ...
    abs(ds_err_n), dt_bin_um);
[bin_count, x_bin, y_bin] = histcounts2(vessel_recon_mask_dt, abs(ds_err_n));
    %% Plot
    fig_hdl = figure;
    ax_hdl = axes(fig_hdl);
...
imagesc(ax_hdl_1, pO2_n_result_ln_2.pO2_array(:, :, vis_sec));
...
imagesc(ax_hdl_2, pO2_n_result_ds2_ln.pO2_array_us(:, :, vis_sec));
ax_hdl_2.DataAspectRatio = [1,1,1];
colorbar(ax_hdl_2);
ax_hdl_2.Colormap(end, :) = [1,0,0];
ax_hdl_2.Title.String = '2 \mum resolution (linear upsampled)';
ax_hdl_2.XLabel.String = 'X (\mum)';
ax_hdl_2.YLabel.String = 'Y (\mum)';

ax_hdl_3 = subplot(1,3,3);
imagesc(ax_hdl_3, pO2_n_result_ds2_ln.pO2_array_us(:, :, vis_sec) - pO2_n_result_ln_2.pO2_array(:, :, vis_sec));
ax_hdl_3.DataAspectRatio = [1,1,1];
colorbar(ax_hdl_3);
ax_hdl_3.Colormap(end, :) = [1,0,0];
ax_hdl_3.Title.String = '2 \mum resolution - 1 \mum resolution';
...
[dt_max_info_0, pO2_min_info_0] = fun_simulation_OT_SA_wdw_sz(vessel_recon_mask_dt,...
    pO2_n_result_ln_2.pO2_array, lm_wz_list_um, valid_extrema_mask);
%%
vis_group = 7;
fig_hdl = figure;
...
histogram(ax_hdl_1, tmp_data_1, tmp_bin_edge, 'Normalization', 'count', 'FaceAlpha', 0.5);
hold(ax_hdl_1, 'on');
histogram(ax_hdl_1, tmp_data_2, tmp_bin_edge, 'Normalization', 'count', 'FaceAlpha', 0.5);
...
tmp_bin_edge = -1600 : 100 : 0;
tmp_stat_str_1 = sprintf('Voxel size: 2\\mum\n%s', fun_analysis_basic_stat_str_to_string(fun_analysis_get_basic_statistics(tmp_data_1)));
tmp_stat_str_2 = sprintf('Voxel size: 1\\mum\n%s', fun_analysis_basic_stat_str_to_string(fun_analysis_get_basic_statistics(tmp_data_2)));
histogram(ax_hdl_2, tmp_data_1, tmp_bin_edge, 'Normalization', 'count', 'FaceAlpha', 0.5);
hold(ax_hdl_2, 'on');
histogram(ax_hdl_2, tmp_data_2, tmp_bin_edge, 'Normalization', 'count', 'FaceAlpha', 0.5);
