...
function  [C, W, sigma2, iter, T] =cpd_GRBF(X, Y, beta, lambda, max_it, tol, viz, outliers, fgt, corresp,sigma2,Conn);
...
%while (iter<max_it)  && (sigma2 > 1e-8) %(sigma2 > 1e-8)
    ...
    if viz
        if isempty(Conn), 
            ...
            % @ERHAN
            cla
            gplot3(Conn{1}, X,'LineWidth',4);
            hold on
            gplot3(Conn{2}, Y,'LineWidth',4);
            gplot3(Conn{2}, T,'LineWidth',4);
            drawnow
        end;
    end    
    iter=iter+1;
