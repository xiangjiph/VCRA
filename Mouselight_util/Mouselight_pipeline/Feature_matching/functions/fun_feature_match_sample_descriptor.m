function [des_1_sub, des_2_sub_shift, des_1_label, des_2_label] = fun_feature_match_sample_descriptor(des_1_sub, ...
...
total_num_descriptor_neighbor_search = sqrt(32 * 1024 * 1024 * 1024 / 4);
...
if rm_descriptor_by_pdist2_Q
    if numel(des_1_label) > total_num_descriptor_neighbor_search || ...
        ...
        warning('The neighbor searching require more than 32GB of memory. Terminated');
    else
...
if num_voxel_1 > total_num_descriptor && num_voxel_2 > total_num_descriptor
    sample_ind_1 = 1 : sample_step : num_voxel_1;
    sample_ind_2 = 1 : sample_step : num_voxel_2;
    des_1_sub = des_1_sub(sample_ind_1, :);
    des_1_label = des_1_label(sample_ind_1);
    des_2_sub_shift = des_2_sub_shift(sample_ind_2, :);
    des_2_label = des_2_label(sample_ind_2);
end
...
if size(des_1_sub, 1) > total_num_descriptor
    ...
    des_1_sub = des_1_sub(cat(2, tmp_idx_list_1{1:cutoff_idx}), :);
    des_1_label = repelem(1:cutoff_idx, tmp_seg_length_1(1:cutoff_idx));
end
...
if size(des_2_sub_shift, 1) > total_num_descriptor
    ...
    [~, cutoff_idx] = min(abs(tmp_cumsum_voxel - total_num_descriptor));
    des_2_sub_shift = des_2_sub_shift(cat(2, tmp_idx_list_2{1:cutoff_idx}), :);
    des_2_label = repelem(1:cutoff_idx, tmp_seg_length_2(1:cutoff_idx));
end
