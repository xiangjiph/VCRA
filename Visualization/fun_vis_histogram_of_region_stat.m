function exit_code = fun_vis_histogram_of_region_stat(region_stat, merge_dim)
...
for iter_plot_field = 1 : num_plot_field
    ...
    for iter_cell = 1 : remain_data_numel
        ...
        if isempty(plot_data)
            fprintf('%s is an empty dataset. Skip plotting\n', root_folder);
            break;
        end
        %% Plot        
        ...
        tmp_num_plot = size(tmp_plot_info, 2);
        stat_str = struct;
        ...
        for iter_plot = 1 : tmp_num_plot
            tmp_plot_feature_name = tmp_plot_info{1, iter_plot};
            ...
            [tmp_count, tmp_edge] = fun_analysis_select_histcount_edge_by_percentile(tmp_stat.hist_pdf, ...
                tmp_stat.hist_edge, [5e-3, 1 - 1e-2]);
            tmp_leg_str = fun_analysis_basic_stat_str_to_string(tmp_stat);
            % Save statistical data
            stat_str.(tmp_plot_feature_name) = tmp_stat;
            
            tmp_plot_fn = sprintf('%s_%s.png', tmp_name_string, tmp_plot_info{1, iter_plot});
            tmp_plot_fp = fullfile(root_folder, tmp_plot_field_name, tmp_plot_fn);
            
            % Plot
            fig_hdl = figure('Visible', 'off');
            fig_hdl.Position(3:4) = fig_hdl.Position(3:4) .* 1.5;
            ax_hdl = axes(fig_hdl);
            hist_hdl = histogram(ax_hdl, 'BinCounts', tmp_count, 'BinEdges', tmp_edge, 'FaceColor', [0, 0.4477, 0.7410], 'FaceAlpha', 0.6);
            ax_hdl.XScale = tmp_plot_info{3, iter_plot};
            ax_hdl.YScale = tmp_plot_info{4, iter_plot};
            ax_hdl.XLabel.String = tmp_plot_info{2, iter_plot};
            ax_hdl.YLabel.String = 'PDF';
            ax_hdl.FontWeight = 'bold';
            ax_hdl.FontSize = 14;
            grid(ax_hdl, 'off');
            box(ax_hdl, 'off');
            lg_hdl = legend(ax_hdl, tmp_leg_str);
            ax_hdl.Title.String = region_stat.info.region_name;
            fun_print_image_in_several_formats(fig_hdl, tmp_plot_fp);
            delete(fig_hdl);
        end
