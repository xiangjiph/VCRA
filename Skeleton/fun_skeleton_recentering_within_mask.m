function mask_skl = fun_skeleton_recentering_within_mask(mask_skl, mask_int, mask)
...
mask_skl = padarray(mask_skl, [1,1,1], false);
mask_int = padarray(mask_int, [1,1,1], 0);
mask = padarray(mask, [1,1,1], false);
...
while num_changed_voxel > 0
    ...
    active_voxel_neighbor_ind = bsxfun(@plus, active_voxel_ind', ind_add_coeff_26);
    % If the neighbor is out side the current mask for computing the
    % skeleton, set its value to 0 so the skeleton won't be moved out side
    % the mask. 
    active_voxel_neighbor_dt = mask_int(active_voxel_neighbor_ind) .* cast(mask(active_voxel_neighbor_ind), 'like', mask_int);
    ...
    for tmp_idx = 1 : num_cand_voxel
        ...
        if is_removable_point_Q(tmp_neighbor_LUT_ind)
            % If the voxel can be removed, in the next round, we need to
            % check all its surrounding voxels. Here we add all it's
            % neighbors to the active_voxel_mask
