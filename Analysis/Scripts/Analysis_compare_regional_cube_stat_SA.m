...
local_extrema_window_size_um = wb_data_cell{1}.cube_stat.pO2_data.local_extrema_window_size;
...
vis_lm_wz_idx = 11;
vis_lm_wz_um = local_extrema_window_size_um(vis_lm_wz_idx);
min_in_cc_ratio = 0.75;
min_in_mask_ratio = 1;
min_cap2vsl_vol_ratio = 0.5;
...
[ plot_data.vessel_length_density, plot_data.capillary_length_density, ...
    plot_data.avg_dt_lm, plot_data.avg_u_lm, ...
    plot_data.avg_d, plot_data.avg_u, ...
    plot_data.avg_dt_lm_ptl, plot_data.avg_u_lm_ptl] = deal(cell(num_cc, num_stack, num_region));
for iter_region = 1 : num_region
    for iter_stack = 1 : num_stack
        for iter_cc = 1 : num_cc
            ...
            plot_data.avg_dt_lm{iter_cc, iter_stack, iter_region} = ...
                tmp_cc_data.pO2_data.dt_lm.dt_mean(tmp_240_cube_selected_Q, vis_lm_wz_idx);
            
            plot_data.avg_dt_lm_ptl{iter_cc, iter_stack, iter_region} = ...
                1 - tmp_cc_data.pO2_data.dt_lm.dt_mean_prctile(tmp_240_cube_selected_Q, vis_lm_wz_idx);
            
            plot_data.avg_u_lm{iter_cc, iter_stack, iter_region} = ...
                tmp_cc_data.pO2_data.pO2_lm.pO2_mean(tmp_240_cube_selected_Q, vis_lm_wz_idx);
            
            plot_data.avg_u_lm_ptl{iter_cc, iter_stack, iter_region} = ...
                tmp_cc_data.pO2_data.pO2_lm.pO2_mean_prctile(tmp_240_cube_selected_Q, vis_lm_wz_idx);          
...
avg_info_cell(end+1, :) = {'vessel_length_density', 0 : 0.1 : 2.4, 'vsl_len_den', 'Vessel length density (m/mm^3)', 'linear', 'linear', density_vis_region_list_ind};
avg_info_cell(end+1, :) = {'capillary_length_density', 0 : 0.1 : 2.4, 'cap_len_den', 'Capillary length density (m/mm^3)', 'linear', 'linear', density_vis_region_list_ind};

avg_info_cell(end+1, :) = {'avg_d', 0 : 1 : 30, 'avg_dt', 'Average vessel-tissue distance (\mum)', 'linear', 'linear', density_vis_region_list_ind};

avg_info_cell(end+1, :) = {'avg_dt_lm', 0 : 1.5 : 60, 'avg_dt_lm', 'Average DT local maximum (\mum)', 'linear', 'linear', density_vis_region_list_ind};
avg_info_cell(end+1, :) = {'avg_u_lm', -1800 : 50 : 0, 'avg_u_lm', 'Average normalized pO_2 minimum', 'linear', 'linear', density_vis_region_list_ind};

avg_info_cell(end+1, :) = {'avg_dt_lm_ptl', 10 .^ linspace(-3, log10(0.1), 20), 'avg_dtlm_ptl', '1 - CDF(Average d_{lm})', 'log', 'linear', density_vis_region_list_ind};
avg_info_cell(end+1, :) = {'avg_u_lm_ptl', 10 .^ linspace(-2.5, log10(0.3), 15), 'avg_ulm_ptl', 'CDF(Average u_{lm})', 'log', 'linear', density_vis_region_list_ind};
%%
