function X = correlation_FFT(X,ker, shape, pad_size)
% correlation_FFT do cross correlation of 2/3 dimension based on Fourier Transform
...
% 1. The algorithm might be improved by replacing flipping with complex
% conjugate and ifftshift. The problem is the indexing... Haven't figure
% out how to get the correct result without padding zeros. 
x_size = size(X);
ker_size = size(ker);
...
for idx_dim = 1 : length(size(ker))
    ker = flip(ker, idx_dim);
end
