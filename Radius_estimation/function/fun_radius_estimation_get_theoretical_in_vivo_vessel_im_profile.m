function result_str = fun_radius_estimation_get_theoretical_in_vivo_vessel_im_profile(vsl_r_pxl, bd_layer_th_pxl, ...
    simu_system_half_size, vessel_evaluation_angle, psf_ker_x, psf_ker_y, psf_ker_z, use_GPU_Q)

if nargin < 8
    use_GPU_Q = false;
end
assert(vsl_r_pxl > bd_layer_th_pxl, 'Vessel radius should be greater than boundary layer thickness');
...
vessel_image = convn(convn(convn(single(vessel_object > exp(-1)), ...
    psf_ker_x, 'same'), psf_ker_y, 'same'), psf_ker_z, 'same');
if isa(vessel_image, 'gpuArray')
    vessel_image = gather(vessel_image);
end
% Convolution between RBC tube and the PSF
vessel_object = convn(convn(convn(single(vessel_object > exp(- max(0, vsl_r_pxl - bd_layer_th_pxl) / vsl_r_pxl)),...
    psf_ker_x, 'same'),psf_ker_y, 'same'), psf_ker_z, 'same');
if isa(vessel_object, 'gpuArray')
    vessel_object = gather(vessel_object);
end

vessel_center_position = ceil(block_size/2);

result_str.vessel_image = vessel_image;
result_str.vessel_rbc_tube_image = vessel_object;
